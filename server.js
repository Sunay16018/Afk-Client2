...
process.on('unhandledRejection', (reason) => console.error('Reddedilen İşlem:', reason));
...
function startBot(sid, host, port, user, ver, auto) {
    const s = getS(sid);
    if (s.bots[user]) {
        s.logs[user].push("<b style='color:#e74c3c'>[SİSTEM] Bu bot zaten aktif!</b>");
        return;
    }
    ...
    try {
        ...
        bot.on('login', () => s.logs[user].push("<b style='color:#2ecc71'>[BAĞLANTI] Başarılı! Sunucuya giriş yapıldı.</b>"));
        ...
        bot.on('end', () => {
            ...
            delete s.bots[user];
            if (reconnect) {
                s.logs[user].push("<b style='color:#f1c40f'>[OTOMATİK] Bağlantı koptu, 5sn sonra tekrar denenecek...</b>");
