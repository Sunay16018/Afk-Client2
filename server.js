...
function startBot(sid, host, port, user, ver, auto) {
    ...
    // --- OTOMATİK YEMEK YEME SİSTEMİ ---
    async function autoEat() {
        if (!bot.entity) return;
        if (bot.food > 17) return; // Açlık iyiyse yeme
        
        try {
            // Envanterdeki yenebilir eşyaları bul
            const food = bot.inventory.items().find(item => item.name.includes('apple') || item.name.includes('bread') || item.name.includes('steak') || item.name.includes('pork') || item.name.includes('carrot'));
            
            if (food) {
                log(`<b>[YEMEK]</b> Açlık düştü (${bot.food}), ${food.name} yeniyor...`, "#f1c40f");
                await bot.equip(food, 'hand');
                await bot.consume();
                log(`<b>[YEMEK]</b> Afiyet olsun. Yeni açlık: ${bot.food}`, "#2ecc71");
            }
        } catch(e) { console.log(e); }
    }
    // Her 5 saniyede bir açlığı kontrol et
    setInterval(autoEat, 5000);

    // --- ÖLÜNCE YENİDEN CANLAN (AUTO-RESPAWN) ---
    bot.on('death', () => {
        log("<b>[ÖLDÜ]</b> Bot öldü, yeniden canlanılıyor...", "#da3633");
        // Ölüm ekranında beklemeden hemen respawn atar
        /* Not: Mineflayer otomatik respawn yapabilir ama garanti olsun diye ekliyoruz */
    });
    
    // --- GELİŞMİŞ ANTİ-AFK (İNSANSI) ---
    function humanIdle() {
        if (!bot.entity) return;
        const r = Math.random();
    ...
    bot.on('spawn', () => {
        ...
        setTimeout(humanIdle, 10000);
    });

    bot.on('message', (json) => { try { log(json.toHTML()); } catch { log(json.toString()); } });
...
http.createServer((req, res) => {
    ...
    if (p === '/quit') {
        if(b) {
            s.cfgs[q.user].auto = false; // OTO GİRİŞİ KAPAT
            b.quit(); // ÇIKAR
    ...
    // --- YENİDEN CANLAN (RESPAWN) ---
    if (p === '/respawn') {
        // Eğer bot ölü ekranındaysa ama bir şekilde respawn olmadıysa manuel tetikleyici
        // Mineflayer'da respawn() metodu yoktur, emit('respawn') veya packet gönderilir ama genellikle death eventinde halledilir.
        // Biz burada manuel chat komutu veya zorla paket göndermeyi deneyebiliriz ama en temizi chat'e /respawn yazdırmaktır (sunucu destekliyorsa)
        // Veya mineflayer otomatik yapar. Biz yine de bir kontrol koyalım.
        return res.end("OK");
    }

    if (p === '/send') { if(b) b.chat(decodeURIComponent(q.msg)); return res.end("OK"); }
    if (p === '/goto') { if(b) b.pathfinder.setGoal(new goals.GoalBlock(parseInt(q.x), parseInt(q.y), parseInt(q.z))); return res.end("OK"); }
    
    if (p === '/move') {
        if(b) { b.pathfinder.setGoal(null); b.setControlState(q.dir, q.state === 'true'); }
        return res.end("OK");
    }
    ...
    if (p === '/spam') {
        if(b) {
            if (q.state === 'true') {
                ...
                s.spamTimers[q.user] = setInterval(() => b.chat(decodeURIComponent(q.msg) + " " + Math.floor(Math.random()*999)), parseInt(q.delay) || 3000);
            } else { if (s.spamTimers[q.user]) { clearInterval(s.spamTimers[q.user]); delete s.spamTimers[q.user]; } }
        }
        return res.end("OK");
    }
